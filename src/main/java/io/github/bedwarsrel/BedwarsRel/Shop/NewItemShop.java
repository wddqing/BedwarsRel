        ...
        for (VillagerTrade trade : category.getOffers()) {
            ...
            if (iStack.getType() == Material.ENDER_CHEST && stack.getType() == Material.ENDER_CHEST) {
                return trade;
                    ...
                    iStack.getType() == Material.POTION ||
                            (
                                    (Main.getInstance().getCurrentVersion().startsWith("v1_9") || Main.getInstance().getCurrentVersion().startsWith("v1_10"))
                                            &&
                                            (
                                                    iStack.getType().equals(Material.valueOf("TIPPED_ARROW")) || iStack.getType().equals(Material.valueOf("LINGERING_POTION")) || iStack.getType().equals(Material.valueOf("SPLASH_POTION"))
                                            )
                            )
                    ) {
                PotionMeta iStackMeta = (PotionMeta) iStack.getItemMeta();
                PotionMeta stackMeta = (PotionMeta) stack.getItemMeta();
                if (iStackMeta.toString().equals(stackMeta.toString())) {
                    return trade;
                }
